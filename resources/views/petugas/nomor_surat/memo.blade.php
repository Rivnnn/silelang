...
@section('title', 'Memo')
@section('page-title', 'Memo')

@php
    $showBackButton = true;
    $backUrl = route('petugas.nomor-surat');
    
    // Generate nomor memo otomatis untuk preview
    $userId = auth()->id();
    $idPetugas = str_pad($userId, 2, '0', STR_PAD_LEFT);
    
    // Urutan global (dari semua petugas)
    $totalGlobal = \App\Models\Memo::count();
    $urutanBerikutnya = $totalGlobal + 1;
    
    $nomorBerikutnya = $idPetugas . "/" . str_pad($urutanBerikutnya, 3, '0', STR_PAD_LEFT) . "-2/BSI-BDG KOTA";
@endphp
...
<div class="box">
    ...
    <!-- Preview Nomor Memo Berikutnya -->
    <div class="nomor-preview">
        <div class="icon">🔢</div>
        <div class="content">
            <strong>Nomor Memo Berikutnya:</strong>
            ...
            <div class="info">
                ...
                <span class="info-highlight">-2</span> = Kode Memo
            </div>
        </div>
    </div>

    <form method="POST" action="{{ route('petugas.surat.memo.store') }}">
        @csrf
        <div class="form-grid">
            ...
            <div class="form-group form-full">
                ...
                <input type="text" name="perihal" placeholder="Masukkan perihal memo" required>
            </div>
            
            <div class="form-group form-full">
                <label>Tujuan <span class="required">*</span></label>
                <input type="text" name="tujuan" placeholder="Masukkan tujuan memo" required>
            </div>

            <div class="form-full">
                <button type="submit" class="btn-primary">💾 Simpan Memo</button>
            </div>
        </div>
    </form>
</div>
                ...
                <tr>
                    <td colspan="6">
                        <div class="empty-state">
                            ...
                            <p>Silakan tambahkan memo baru menggunakan form di atas</p>
                        </div>
                    </td>
                </tr>
                @endforelse
